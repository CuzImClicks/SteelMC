...
fn parse_smelting_like_recipe(recipe_name: &str, data: &RecipeJson) -> Option<SmeltingRecipeData> {
...
fn generate_stonecutting_body(r: StonecuttingRecipeData) -> TokenStream {
    let name = &r.name;
    let result_item_ident = &r.result;
...
pub(crate) fn build() -> TokenStream {
    ...
    #[expect(clippy::too_many_arguments, reason = "its fine")]
    fn read_recipes(
        dir: &Path,
        ...
        for entry in fs::read_dir(dir).unwrap() {
            ...
            if path.is_dir() {
                ...
                match recipe.recipe_type.as_str() {
                    ...
                    "minecraft:smelting" => {
                        if let Some(r) = parse_smelting_like_recipe(recipe_name, &recipe) {
                            smelting.push(r);
                        }
                    }
    ...
    let stonecutting = generate_codegen(
        "stonecutting",
        "StonecuttingRecipe",
        &stonecutting_recipes
            .into_iter()
            .map(|recipe| (recipe.ident.clone(), generate_stonecutting_body(recipe)))
            .collect::<Vec<(Ident, TokenStream)>>(),
    );
    ...
    let all_creator_fns: Vec<&TokenStream> = shaped
        ...
        .chain(blasting.creator_fns.iter())
        .chain(stonecutting.creator_fns.iter())
        .collect();
    ...
    let all_registers: Vec<&TokenStream> = shaped
        ...
        .chain(blasting.registers.iter())
        .chain(stonecutting.registers.iter())
        .collect();
    ...
    let blasting_fields = &blasting.fields;
    let blasting_field_inits = &blasting.field_inits;

    let stonecutting_fields = &stonecutting.fields;
    let stonecutting_field_inits = &stonecutting.field_inits;
    ...
    quote! {
        ...
        pub struct BlastingRecipes { #(#blasting_fields)* }
        pub struct StonecuttingRecipes { #(#stonecutting_fields)* }
